...
public class CrepeOnnxSimd : IF0Predictor, IDisposable
{
        ...
        for (var i = 0; i < batchSize; i++)
        {
            ...
            for (var v = 0; v < vectorCount; v++)
            {
                ...
                var source = new Vector<float>(
                    _inputBatchBuffer.AsSpan(inputOffset + vectorOffset, _vectorSize)
                );
    ...
    private void ApplyFrequencyRangeLimit(
        float[] probabilities,
        int outputSize,
        int minIdx,
        int maxIdx
        ...
        Parallel.For(
            ...
            threadIdx =>
            {
                ...
                for (var j = startState; j < endState; j++)
                {
                    ...
                    {
                        var transVal =
                            _valueBuffer[baseIdxPrev + k] + _transitionMatrix[j * PITCH_BINS + k];
                        if (transVal > maxVal)
                        {
                            maxVal = transVal;
                            maxI = k;
    ...
    private void MedianFilter(Span<float> data, int windowSize)
    {
        if (windowSize > _medianBuffer.Length || windowSize % 2 == 0)
        {
            throw new ArgumentException(
                "Window size must be odd and <= buffer size",
                nameof(windowSize)
            );
        }
