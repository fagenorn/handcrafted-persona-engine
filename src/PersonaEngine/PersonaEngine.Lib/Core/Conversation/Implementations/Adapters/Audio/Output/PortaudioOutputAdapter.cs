...
public class PortaudioOutputAdapter(
    ILogger<PortaudioOutputAdapter> logger,
    IAudioProgressNotifier progressNotifier
) : IAudioOutputAdapter
{
    ...
    public ValueTask InitializeAsync(Guid sessionId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            logger.LogDebug(
                "Using PortAudio output device: {DeviceName} (Index: {DeviceIndex})",
                deviceInfo.name,
                deviceIndex
            );
        ...
        catch (Exception ex)
        {
            ...
            throw new AudioPlayerInitializationException(
                "Failed to initialize PortAudio audio system.",
                ex
            );
        }
    ...
    )
    {
        ...
        _progressChannel = Channel.CreateBounded<IAudioProgressEvent>(
        ...
        try
        {
            ...
            while (await inputReader.WaitToReadAsync(localPlaybackCts.Token).ConfigureAwait(false))
            {
                ...
                var firstChunkEvent = new AudioPlaybackStartedEvent(
                    _sessionId,
                    turnId,
                    DateTimeOffset.UtcNow
                );
                await outputWriter
                    .WriteAsync(firstChunkEvent, localPlaybackCts.Token)
        ...
        catch (Exception ex)
        {
            ...
            await outputWriter
                .WriteAsync(
                    new ErrorOutputEvent(_sessionId, turnId, DateTimeOffset.UtcNow, ex),
