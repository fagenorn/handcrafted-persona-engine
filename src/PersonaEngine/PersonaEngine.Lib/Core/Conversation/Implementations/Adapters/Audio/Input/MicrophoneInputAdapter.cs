...
) : IAudioInputAdapter
{
    private readonly AsyncRetryPolicy _transcriptionRetyPolicy = Policy
        ...
        .WaitAndRetryAsync(
            Backoff.ExponentialBackoff(TimeSpan.FromMilliseconds(1000), 3),
            async (exception, timespan, retryAttempt, context) =>
            {
                var sessionId = context.TryGetValue("sessionId", out var value)
                    ? (Guid)value
                    : Guid.Empty;
                logger.LogWarning(
                    "Transcription failed/stopped for session {SessionId}. Retrying in {Timespan}. Attempt {RetryAttempt}...",
                    sessionId,
                    timespan,
                    retryAttempt
                );
    ...
    public ParticipantInfo Participant { get; } =
        new(Guid.NewGuid().ToString(), "User", ChatMessageRole.User);

    public IAwaitableAudioSource GetAudioSource()
    {
        return microphone;
    }
    ...
    public ValueTask StartAsync(CancellationToken cancellationToken)
    {
        ...
        _transcriptionTask = Task.Run(
            async () => await TranscriptionJobWithRetry(localSessionId, localWriter, localCt),
            localCt
        );
    ...
    public async ValueTask StopAsync(CancellationToken cancellationToken)
    {
        ...
        if (_transcriptionTask != null)
        {
            ...
            catch (Exception ex)
            {
                logger.LogError(
                    ex,
                    "Exception waiting for transcription task to complete during stop: {Message}",
                    ex.Message
                );
            }
            finally
    ...
    private async ValueTask TranscriptionJobWithRetry(
        Guid sessionId,
        ChannelWriter<IInputEvent> writer,
        CancellationToken ct
    )
    {
        logger.LogDebug("Transcription job starting for session {SessionId}.", sessionId);
        var pollyContext = new Polly.Context($"TranscriptionJob_{sessionId}")
        {
            ["sessionId"] = sessionId,
        };
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            logger.LogError(
                ex,
                "Unhandled error during transcription job for session {SessionId} after retries (if any): {Message}",
                sessionId,
                ex.Message
            );
        }
        finally
        {
            logger.LogInformation(
                "Transcription job execution flow completed for session {SessionId}.",
                sessionId
            );
        }
    }

    private async ValueTask RunTranscriptionLoop(
        Guid sessionId,
        ChannelWriter<IInputEvent> writer,
        CancellationToken ct
    )
    ...
    private IInputEvent? MapToInputEvent(
        IRealtimeTranscriptionSegment transcriptionEvent,
        Guid sessionId
    )
    {
        return transcriptionEvent switch
        {
            ...
            RealtimeSegmentRecognizing recognizing => new SttSegmentRecognizing(
                ...
                recognizing.ProcessingDuration
            ),
    ...
    private class RestartTranscriptionException()
        : Exception("Transcription session stopped and needs restart.");
}
