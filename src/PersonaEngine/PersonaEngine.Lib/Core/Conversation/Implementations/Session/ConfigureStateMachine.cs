...
public partial class ConversationSession
{
    ...
    private StateMachine<
        ConversationState,
        ConversationTrigger
    >.TriggerWithParameters<IInputEvent> _inputDetectedTrigger = null!;

    private StateMachine<
        ConversationState,
        ConversationTrigger
    >.TriggerWithParameters<IInputEvent> _inputFinalizedTrigger = null!;

    private void ConfigureStateMachine()
    {
        _inputDetectedTrigger = _stateMachine.SetTriggerParameters<IInputEvent>(
            ConversationTrigger.InputDetected
        );
        _inputFinalizedTrigger = _stateMachine.SetTriggerParameters<IInputEvent>(
            ConversationTrigger.InputFinalized
        );

        var llmChunkReceivedTrigger = _stateMachine.SetTriggerParameters<IOutputEvent>(
            ConversationTrigger.LlmStreamChunkReceived
        );
        var ttsChunkReceivedTrigger = _stateMachine.SetTriggerParameters<IOutputEvent>(
            ConversationTrigger.TtsStreamChunkReceived
        );
        var errorOccurredTrigger = _stateMachine.SetTriggerParameters<Exception>(
        ...
        _stateMachine
            .Configure(ConversationState.Listening)
        ...
        _stateMachine
            .Configure(ConversationState.ActiveTurn)
            .PermitIf(
                _inputDetectedTrigger,
                ConversationState.Interrupted,
                ShouldAllowBargeIn,
                "Barge-In"
            )
            .PermitIf(
                _inputFinalizedTrigger,
                ConversationState.Interrupted,
                ShouldAllowBargeIn,
                "Barge-In"
            );
        ...
        _stateMachine
            .Configure(ConversationState.WaitingForLlm)
            .SubstateOf(ConversationState.ActiveTurn)
        ...
        _stateMachine
            .Configure(ConversationState.StreamingResponse)
            .SubstateOf(ConversationState.ActiveTurn)
        ...
        _stateMachine
            .Configure(ConversationState.Speaking)
            .SubstateOf(ConversationState.ActiveTurn)
        ...
        _stateMachine
            .Configure(ConversationState.Interrupted)
            .OnEntryFromAsync(
                _inputDetectedTrigger,
                HandleInterruptionCancelAndRefireAsync,
                "Handle Detected Interruption, Cancel, Refire"
            )
            .OnEntryFromAsync(
                _inputFinalizedTrigger,
                HandleInterruptionCancelAndRefireAsync,
                "Handle Finalized Interruption, Cancel, Refire"
            )
        ...
        _stateMachine
            .Configure(ConversationState.Error)
            .OnEntryFromAsync(
                errorOccurredTrigger,
                HandleErrorAsync,
        ...
        _stateMachine
            .Configure(ConversationState.Ended)
        ...
        _stateMachine.OnUnhandledTriggerAsync(
            async (state, trigger, unmetGuards) =>
            {
                if (
                    _stateMachine.IsInState(ConversationState.Ended)
                    || _stateMachine.IsInState(ConversationState.Error)
                )
                {
                    return;
                }
                ...
                _logger.LogWarning(
                    "{SessionId} | Unhandled trigger '{Trigger}' in state '{State}'. Unmet guards: [{UnmetGuards}]",
                    SessionId,
                    trigger,
                    state,
                    string.Join(", ", unmetGuards ?? Array.Empty<string>())
                );

                await _stateMachine.FireAsync(
                    ConversationTrigger.ErrorOccurred,
                    new InvalidOperationException($"Unhandled trigger {trigger} in state {state}")
                );
            }
        );
        ...
        {
            _logger.LogDebug(
                "{SessionId} | Transitioned: {SourceState} --({Trigger})--> {DestinationState}",
                SessionId,
                transition.Source,
                transition.Trigger,
    ...
    )
    {
        ...
        _logger.LogInformation(
            "{SessionId} | {TurnId} | {Emoji} | {DestinationState}",
            SessionId,
    ...
    private bool ShouldAllowBargeIn(IInputEvent inputEvent)
    {
        ...
        _logger.LogDebug(
            "{SessionId} | Barge-in check for trigger {TriggerType} in state {State}. Strategy decided: {Allow}",
            SessionId,
