...
public partial class ConversationSession : IConversationSession
{
        ...
        try
        {
            ...
            )
            {
                ...
                try
                {
                    ...
                    {
                        var sttDuration = (
                            sttRecognized.Timestamp - _sttStartTime.Value
                        ).TotalMilliseconds;
                    ...
                    switch (outputEvent)
                    {
                        ...
                        case AudioPlaybackStartedEvent ev:
                            if (_firstAudioLatencyStopwatch != null && ev.TurnId.HasValue)
                            {
                                ...
                                _currentTurnFirstAudioLatencyMs = _firstAudioLatencyStopwatch
                                    .Elapsed
                                    .TotalMilliseconds;
                                _metrics.RecordFirstAudioLatency(
                                    _firstAudioLatencyStopwatch.Elapsed.TotalMilliseconds,
                                    SessionId,
                                    ev.TurnId.Value
                                );
                        ...
                        case AudioPlaybackEndedEvent ev:
                            if (_audioPlaybackStopwatch != null)
                            {
                                ...
                                if (_turnStopwatch != null && ev.TurnId.HasValue)
                                {
                                    ...
                                    if (
                                        _ttsFinishReason == CompletionReason.Error
                                        || _llmFinishReason == CompletionReason.Error
                                    )
                                    {
                                        overallReason = CompletionReason.Error;
                                    }
                                    else if (
                                        _ttsFinishReason == CompletionReason.Cancelled
                                        || _llmFinishReason == CompletionReason.Cancelled
                                    )
                                    {
    ...
    private (ConversationTrigger? Trigger, object? EventArgs) MapInputEventToTrigger(
    ...
    private (ConversationTrigger? Trigger, object? EventArgs) MapOutputEventToTrigger(
    ...
    // Communication Channels (Turn-specific)
    ...
    private double? _currentTurnFirstSttChunkLatencyMs;
    ...
    private async Task PrepareLlmRequestAsync(IInputEvent inputEvent)
    {
        ...
        _currentTurnFirstSttChunkLatencyMs = finalizedEvent.ProcessingDuration.TotalMilliseconds;
    ...
    private async Task CancelCurrentTurnProcessingAsync()
    {
        ...
        _logger.LogDebug(
            "{SessionId} | Requesting cancellation for Turn {TurnId}'s processing pipeline.",
            SessionId,
        ...
        if (_currentTurnCts is { IsCancellationRequested: false })
        {
            try
            {
                ...
                _logger.LogTrace(
                    "{SessionId} | Turn {TurnId} CancellationToken cancelled.",
                    SessionId,
                    turnIdBeingCancelled
                );
            }
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "{SessionId} | Error cancelling CancellationTokenSource for Turn {TurnId}.",
                    SessionId,
                    turnIdBeingCancelled
                );
            }
    ...
    )
    {
        ...
        {
            _logger.LogTrace(
                "{SessionId} | Waiting briefly for {TaskName} task cancellation acknowledgment (Turn {TurnId}).",
                SessionId,
                taskName,
                turnIdBeingCancelled
            );
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "{SessionId} | Exception while waiting for {TaskName} task cancellation (Turn {TurnId}). Task Status: {Status}",
                    SessionId,
                    taskName,
                    turnIdBeingCancelled,
                    task.Status
        ...
        {
            _logger.LogTrace(
                "{SessionId} | {TaskName} task was null or already completed (Turn {TurnId}). No wait needed.",
                SessionId,
                taskName,
                turnIdBeingCancelled
            );
    ...
    private void HandleLlmStreamRequested()
    {
        ...
        {
            _logger.LogError(
                "{SessionId} | HandleLlmStreamRequested called in invalid state (TurnId: {TurnId}, Cts: {Cts}).",
                SessionId,
                turnId,
                turnCts != null
            );
            ...
            _ = FireErrorAsync(
                new InvalidOperationException("Cannot start LLM stream in invalid state."),
                false
            );
        ...
        _currentLlmChannel = Channel.CreateUnbounded<LlmChunkEvent>(
            new UnboundedChannelOptions { SingleReader = true, SingleWriter = false }
        );
    ...
    private void HandleTtsStreamRequest()
    {
        ...
        {
            _logger.LogWarning(
                "{SessionId} | HandleTtsStreamRequest called without a valid TurnId.",
                SessionId
            );
        ...
        _currentTtsChannel = Channel.CreateUnbounded<TtsChunkEvent>(
            new UnboundedChannelOptions { SingleReader = true, SingleWriter = false }
        );
    ...
    private void TrackLlmFirstTokenLatency(Guid turnId)
    {
        ...
        _firstLlmTokenLatencyStopwatch.Stop();
        _currentTurnFirstLlmTokenLatencyMs = _firstLlmTokenLatencyStopwatch
            .Elapsed
            .TotalMilliseconds;
        ...
        _logger.LogDebug(
            ...
            SessionId,
            turnId,
            _currentTurnFirstLlmTokenLatencyMs.Value
        );
    ...
    )
    {
        ...
        {
            _firstTtsChunkLatencyStopwatch.Stop();
            _currentTurnFirstTtsChunkLatencyMs = _firstTtsChunkLatencyStopwatch
                .Elapsed
                .TotalMilliseconds;
            _logger.LogDebug(
                ...
                SessionId,
                turnId.Value,
                _currentTurnFirstTtsChunkLatencyMs.Value
            );
            _firstTtsChunkLatencyStopwatch = null;
    ...
    private void HandleLlmStreamEnded()
    {
        ...
        {
            _logger.LogWarning(
                "{SessionId} | HandleLlmStreamEnded called without a valid LLM channel.",
                SessionId
            );
    ...
    private void HandleTtsStreamEnded()
    {
        ...
        {
            _logger.LogWarning(
                "{SessionId} | HandleTtsStreamEnded called without a valid TTS channel.",
                SessionId
            );
    ...
    private void HandleIdle()
    {
        ...
        {
            var sttTime = _currentTurnFirstSttChunkLatencyMs ?? 0;
