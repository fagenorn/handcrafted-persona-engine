...
public sealed class VBridgerLipSyncService : AnimationServiceBase
{
    ...
    private void InitializeCurrentParameters()
    {
        ...
        _currentParameterValues[ParamMouthPuckerWiden] = cubismModel.GetParameterValue(
            ParamMouthPuckerWiden
        );
        _currentParameterValues[ParamMouthPressLipOpen] = cubismModel.GetParameterValue(
        ...
        if (foundIndex != -1)
        {
            if (foundIndex != _currentPhonemeIndex)
            {
                Logger.LogTrace(
                    ...
                    foundIndex,
                    _activePhonemes[foundIndex].Phoneme,
                    currentTime
                );
                ...
                {
                    _currentTargetPose = GetPoseForPhoneme(
                        _activePhonemes[_currentPhonemeIndex].Phoneme
                    );
                }
    ...
    private void SmoothParametersToTarget(PhonemePose targetPose, float deltaTime)
    {
        ...
        _currentParameterValues[ParamMouthOpenY] = Lerp(
            _currentParameterValues[ParamMouthOpenY],
            targetPose.MouthOpenY,
            smoothFactor
        );
        _currentParameterValues[ParamJawOpen] = Lerp(
            _currentParameterValues[ParamJawOpen],
            targetPose.JawOpen,
            smoothFactor
        );
        _currentParameterValues[ParamMouthForm] = Lerp(
            _currentParameterValues[ParamMouthForm],
            targetPose.MouthForm,
            smoothFactor
        );
        _currentParameterValues[ParamMouthShrug] = Lerp(
            _currentParameterValues[ParamMouthShrug],
            targetPose.MouthShrug,
            smoothFactor
        );
        _currentParameterValues[ParamMouthFunnel] = Lerp(
            _currentParameterValues[ParamMouthFunnel],
            targetPose.MouthFunnel,
            smoothFactor
        );
        _currentParameterValues[ParamMouthPuckerWiden] = Lerp(
            _currentParameterValues[ParamMouthPuckerWiden],
            targetPose.MouthPuckerWiden,
            smoothFactor
        );
        _currentParameterValues[ParamMouthPressLipOpen] = Lerp(
            _currentParameterValues[ParamMouthPressLipOpen],
            targetPose.MouthPressLipOpen,
            smoothFactor
        );
        _currentParameterValues[ParamMouthX] = Lerp(
            _currentParameterValues[ParamMouthX],
            targetPose.MouthX,
            smoothFactor
        );
        ...
        {
            _currentParameterValues[ParamCheekPuffC] = Lerp(
                _currentParameterValues[ParamCheekPuffC],
                targetPose.CheekPuffC,
                smoothFactor
            );
        }
        else
    ...
    private void SmoothParametersToNeutral(float deltaTime)
    {
        ...
        _currentParameterValues[ParamMouthPuckerWiden] = Lerp(
            _currentParameterValues[ParamMouthPuckerWiden],
            neutral.MouthPuckerWiden,
            smoothFactor
        );
        _currentParameterValues[ParamMouthPressLipOpen] = Lerp(
            _currentParameterValues[ParamMouthPressLipOpen],
            neutral.MouthPressLipOpen,
            smoothFactor
        );
        _currentParameterValues[ParamMouthX] = Lerp(
            _currentParameterValues[ParamMouthX],
    ...
    private bool IsApproximatelyNeutral()
    {
        ...
        return Math.Abs(_currentParameterValues[ParamMouthOpenY] - neutral.MouthOpenY)
                < NEUTRAL_THRESHOLD
            && Math.Abs(_currentParameterValues[ParamJawOpen] - neutral.JawOpen) < NEUTRAL_THRESHOLD
            && Math.Abs(_currentParameterValues[ParamMouthForm] - neutral.MouthForm)
                < NEUTRAL_THRESHOLD
            && Math.Abs(_currentParameterValues[ParamMouthShrug] - neutral.MouthShrug)
                < NEUTRAL_THRESHOLD
            && Math.Abs(_currentParameterValues[ParamMouthFunnel] - neutral.MouthFunnel)
                < NEUTRAL_THRESHOLD
            && Math.Abs(_currentParameterValues[ParamMouthPuckerWiden] - neutral.MouthPuckerWiden)
                < NEUTRAL_THRESHOLD
            && Math.Abs(_currentParameterValues[ParamMouthPressLipOpen] - neutral.MouthPressLipOpen)
                < NEUTRAL_THRESHOLD
            && Math.Abs(_currentParameterValues[ParamMouthX] - neutral.MouthX) < NEUTRAL_THRESHOLD
            && Math.Abs(_currentParameterValues[ParamCheekPuffC] - neutral.CheekPuffC)
                < NEUTRAL_THRESHOLD;
    }
    ...
    public struct PhonemePose : IEquatable<PhonemePose>
    {
        ...
        {
            return obj is PhonemePose other && Equals(other);
        }
        ...
        public static bool operator ==(PhonemePose left, PhonemePose right)
        {
    ...
    private Dictionary<string, PhonemePose> InitializeMisakiPhonemeMap()
    {
        ...
        ); // 'r': Some funneling, slight pucker, separation
        ...
        ); // 'oo': Funnel, puckered, slight press
