...
public class SemanticKernelChatEngine : IChatEngine
{
    ...
    )
    {
        ...
        try
        {
                ...
                .ExecuteAsync(
                    async (x, token) =>
                    {
                        ...
                        await foreach (var chunk in streamingResponse.ConfigureAwait(false))
                        {
                            ...
                            if (firstChunk)
                            {
                                var firstChunkEvent = new LlmStreamStartEvent(
                                    x.sessionId,
                                    x.turnId,
                                    DateTimeOffset.UtcNow
                                );
                                await x
                                    .outputWriter.WriteAsync(firstChunkEvent, token)
                                    .ConfigureAwait(false);
                                firstChunk = false;
                            }

                            var chunkEvent = new LlmChunkEvent(
                                x.sessionId,
                                x.turnId,
                                DateTimeOffset.UtcNow,
                                content
                            ...
                            await x
                                .outputWriter.WriteAsync(chunkEvent, token)
                                .ConfigureAwait(false);

                            _logger.LogTrace(
                                "Received chunk {ChunkNumber}: {ChunkLength} characters for TurnId: {TurnId}",
                                chunkCount,
                                content.Length,
                                x.turnId
                            );
                        ...
                        _logger.LogInformation(
                            "Chat response streaming completed. Total chunks: {ChunkCount}, Total time: {ElapsedMs}ms",
                            chunkCount,
                            stopwatch.ElapsedMilliseconds
                        );
                    },
                    (stopwatch, outputWriter, history, executionSettings, sessionId, turnId),
                    cancellationToken
                )
                .ConfigureAwait(false);
            ...
            {
                _logger.LogWarning(
                    "Chat response streaming finished for TurnId: {TurnId}, but no chunks were received (potentially empty response or immediate failure). Time: {ElapsedMs}ms",
                    turnId,
                    stopwatch.ElapsedMilliseconds
                );
            }
        ...
        finally
        {
            if (!firstChunk)
            {
                var endEvent = new LlmStreamEndEvent(
                    sessionId,
                    turnId,
                    DateTimeOffset.UtcNow,
                    completedReason
                ...
                catch (Exception finalEx)
                {
                    _logger.LogError(
                        finalEx,
                        "Failed to write LlmStreamEndEvent for TurnId: {TurnId}",
                        turnId
                    );
                }
            }
