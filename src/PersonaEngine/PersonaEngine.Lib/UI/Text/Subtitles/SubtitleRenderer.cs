...
public class SubtitleRenderer : IRenderComponent
{
    ...
    public void Update(float deltaTime)
    {
        ...
        _linesToRender = _subtitleTimeline.GetVisibleLinesAndPosition(
            currentTime,
            _viewportWidth,
            _viewportHeight
        );
        ...
        foreach (var line in _linesToRender)
        {
            foreach (var word in line.Words)
            {
                if (
                    ...
                    || word.IsActive((float)_currentPlaybackTime.TotalSeconds)
                )
                {
    ...
    {
        _subtitleTimeline.RemoveSegment(e.Chunk);
    }

    private void OnPlaybackProgress(object? sender, AudioPlaybackProgressEvent e)
    {
        _currentPlaybackTime = e.CurrentPlaybackTime;
    }
