...
public class ChatEditor : ConfigSectionEditorBase
{
    ...
    private IReadOnlyDictionary<string, ParticipantInfo> _participantsSnapshot =
        new Dictionary<string, ParticipantInfo>();
    ...
    public override void Render()
    {
        ...
        ImGui.BeginChild(
            "ParticipantsPanel",
            new Vector2(_participantsPanelWidth, 700),
    ...
    )
    {
        ...
        for (var msgIndex = 0; msgIndex < turn.Messages.Count; msgIndex++)
        {
            ...
            var textSize = ImGui.CalcTextSize(
                messageText,
                false,
                maxBubbleWidth - style.WindowPadding.X * 2
            );
            var nameSize = ImGui.CalcTextSize(
            ...
            var bubbleHeight =
                ...
                + (isUserMessage ? ImGui.GetTextLineHeight() : 0);

            var startPosX = style.WindowPadding.X;
            if (isUserMessage)
            {
                startPosX =
                    availableWidth - bubbleWidth - style.WindowPadding.X - style.ScrollbarSize;
            }
