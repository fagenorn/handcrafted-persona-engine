...
public sealed class SessionStatsCollectorTests : IDisposable
{
    ...
    [Fact]
    public void AvgFirstAudioLatency_computes_running_average()
    {
        var sessionId = Guid.NewGuid();

        _metrics.RecordFirstAudioLatency(100.0, sessionId, Guid.NewGuid());
        ...
        Assert.Equal(200.0, _collector.AvgFirstAudioLatencyMs!.Value, precision: 1);
    }

    [Fact]
    public void AvgFirstAudioLatency_single_recording()
    {
        var sessionId = Guid.NewGuid();
